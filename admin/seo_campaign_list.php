                                                        ...
                                                        <div class="col-md-4 my-2 my-md-0">
                                                            <div class="form-group">
                                                                ...
                                                                <select id="BG_SourceFilter"
                                                                    ...
                                                                    <?php
                                                                    $select = "SELECT s.id, s.name FROM sources AS s INNER JOIN seo_campaigns AS c ON s.id=c.source_id WHERE s.type='{$type}' AND  s.company_id='{$global_company_id}' AND s.branch_id='{$global_branch_id}' AND s.deleted_at IS NULL AND  c.company_id='{$global_company_id}' AND c.branch_id='{$global_branch_id}' AND c.deleted_at IS NULL GROUP BY s.id ORDER BY s.sort_by ASC";
...
<script>
    ...
    function getAllPageData(filter) {
        loader(true);
        $.ajax({
            type: "POST", url: 'ajax/fetch/seo_campaigns.php',
            data: {'filters': filter},
            ...
            if (result.value) {
                ...
                if (D) {
                    ...
                    $.ajax({
                        ...
                        data: "delete_seo_campaign=" + id + "&user_right_title=<?php echo $user_right_title; ?>",
                        success: function (resPonse) {
                            if (resPonse !== undefined && resPonse != '') {
                                var obj = JSON.parse(resPonse);
