...
if (isset($_POST['filters']) && !empty($_POST['filters'])) {
    ...
    $sortColumn = 'name';
    $sortOrder = 'ASC';
    $condition = " WHERE a.type='{$filters->T}' AND s.type='{$filters->T}' AND a.company_id='{$global_company_id}' AND s.company_id='{$global_company_id}' AND a.branch_id='{$global_branch_id}' AND s.branch_id='{$global_branch_id}' AND a.deleted_at IS NULL AND s.deleted_at IS NULL ";
    if (isset($filters->SearchQuery) && !empty($filters->SearchQuery) && strlen($filters->SearchQuery) > 0) {
        $condition .= " AND (a.name LIKE '%{$filters->SearchQuery}%' OR s.name LIKE '%{$filters->SearchQuery}%') ";
    }
    ...
    $total = 0;
    $data = '';
    $sql = mysqli_query($db, "SELECT count(a.id) AS total FROM accounts AS a INNER JOIN sources AS s ON a.source_id = s.id " . $condition);
    ...
    if ($total > 0) {
        ...
        $select = "SELECT a.id, a.name, s.name AS source_name
        FROM 
            accounts AS a
        INNER JOIN
            sources AS s
            ON a.source_id = s.id " . $condition . $sort . $number_of_record;
                    ...
                    if (hasRight($filters->L, 'edit') || hasRight($filters->L, 'delete')) {
                        if (isset($filters->Actions) && !empty($filters->Actions) && sizeof($filters->Actions) > 0) {
                            ...
                            if (hasRight($filters->L, 'edit'))
                                $data .= '<a href="' . $admin_url . $filters->T . '_account?id=' . $result['id'] . '" class="btn btn-sm btn-clean btn-icon" title="Edit"><i class="la la-edit"></i></a>';
