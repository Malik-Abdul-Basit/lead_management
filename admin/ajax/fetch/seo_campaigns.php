...
if (isset($_POST['filters']) && !empty($_POST['filters'])) {
    ...
    if (isset($filters->SearchQuery) && !empty($filters->SearchQuery) && strlen($filters->SearchQuery) > 0) {
        $condition .= " AND (c.name LIKE '%{$filters->SearchQuery}%' OR c.cost LIKE '%{$filters->SearchQuery}%' OR c.reach LIKE '%{$filters->SearchQuery}%' OR c.clicks LIKE '%{$filters->SearchQuery}%' OR c.form_submissions LIKE '%{$filters->SearchQuery}%' OR c.calls LIKE '%{$filters->SearchQuery}%') ";
    }
    ...
    if (isset($filters->DateRange) && !empty($filters->DateRange) && sizeof($filters->DateRange) > 0) {
        ...
        if (!empty($range_object->rangeStart)) {
            ...
            $condition .= " AND (c.from BETWEEN '" . $rangeStart . "' and '" . $rangeEnd . "' OR c.to BETWEEN '" . $rangeStart . "' and '" . $rangeEnd . "') ";
        }
    }
    ...
    $total = 0;
    $data = '';
    $sql = mysqli_query($db, "SELECT count(c.id) AS total FROM seo_campaigns AS c INNER JOIN sources AS s ON c.source_id=s.id " . $condition);
    ...
    if (isset($filters->L)) {
        if (hasRight($filters->L, 'edit') || hasRight($filters->L, 'delete')) {
            $h_col = '<div class="col-md-3"><b>Source</b></div><div class="col-md-2"><b>Action</b></div>';
            $right = true;
        } else {
            $h_col = '<div class="col-md-5"><b>Source</b></div>';
            $right = false;
        }
    }
                    ...
                    <div class="collapse-card-outer-wrapper">
                        <div class="table-header">
                            <div class="row">
                                ...
                                <div class="col-md-3"><b>Date</b></div>' . $h_col . '
                            </div>
                        </div>
                    </div>
                </th>
                ...
                $data .= '<div class="collapse-card-outer-wrapper">
                    <div class="collapse-card">
                        <div class="card-pane success">
                            <div class="row">
                                <div class="col-md-4 text-vertical-align-center">
                                ...
                                <div class="col-md-3 text-vertical-align-center">
                                    <div class="flex-grow-0">
                                        <span>' . date('d-M-Y', strtotime($result->from)) . '</span>
                                        <small class="d-block text-center">To</small>
                                        <span>' . date('d-M-Y', strtotime($result->to)) . '</span>
                                    </div>
                                </div>';
                if ($right) {
