...
if (isset($_POST['filters']) && !empty($_POST['filters'])) {
    ...
    if ($employee_info->user_type != config('users.type.value.super_admin') && $employee_info->user_type != config('users.type.value.admin') ) {
        $condition .= " AND l.added_by='{$global_user_id}'";
    }

    $total = 0;
    $data = '';
    $sql = mysqli_query($db, "SELECT count(l.id) AS total FROM tele_marketing_leads AS l INNER JOIN sales_persons AS sp ON sp.id=l.sales_person_id " . $condition . " GROUP BY l.id ");
    ...
    if (isset($filters->L)) {
        if (hasRight($filters->L, 'edit') || hasRight($filters->L, 'delete')) {
            $h_col = '<div class="col-md-3"><b>Status</b></div><div class="col-md-2"><b>Action</b></div>';
            $right = true;
        } else {
            $h_col = '<div class="col-md-5"><b>Status</b></div>';
            $right = false;
        }
    }
                ...
                <th>
                    <div class="collapse-card-outer-wrapper">
                        <div class="table-header">
                            <div class="row">
                                <div class="col-md-5"><b>Sales Person</b></div>
    ...
    if ($total > 0) {
        ...
        FROM 
            tele_marketing_leads AS l 
        INNER JOIN 
            sales_persons AS sp 
            ON sp.id=l.sales_person_id " . $condition . " GROUP BY l.id " . $sort . $number_of_record;
        ...
        if (mysqli_num_rows($query) > 0) {
            ...
            while ($result = mysqli_fetch_object($query)) {
                ...
                $status = config('tele_marketing_leads.status.title.' . $result->status);
                ...
                if ($right) {
                    $data .= '<div class="col-md-3 text-vertical-align-center">' . $status . '</div>';

                    $data .= '<div class="col-md-2 text-vertical-align-center">';
                    if (hasRight($filters->L, 'edit')) {
                        $data .= '<a href="' . $admin_url . 'tele_marketing_lead?id=' . $result->id . '" class="btn btn-sm btn-light-primary font-weight-bolder text-uppercase mr-2" style="font-size: 10px" title="Edit">
                    ...
                    $data .= '<div class="col-md-5 text-vertical-align-center">' . $status . '</div>';
                }
                ...
                $query_lead_messages = mysqli_query($db, "SELECT * FROM `tele_marketing_lead_messages` WHERE `lead_id`='{$result->id}'");
