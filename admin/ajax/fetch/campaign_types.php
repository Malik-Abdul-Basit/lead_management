...
if (isset($_POST['filters']) && !empty($_POST['filters'])) {
    ...
    $sortColumn = 'sort_by';
    $sortOrder = 'ASC';
    $condition = " WHERE `type`='{$filters->T}' AND `company_id`='{$global_company_id}' AND `branch_id`='{$global_branch_id}' AND `deleted_at` IS NULL";
    if (isset($filters->SearchQuery) && !empty($filters->SearchQuery) && strlen($filters->SearchQuery) > 0) {
        $condition .= " AND (`name` LIKE '%{$filters->SearchQuery}%' OR `sort_by` LIKE '%{$filters->SearchQuery}%') ";
    }

    $total = 0;
    $data = '';
    $sql = mysqli_query($db, "SELECT count(id) AS total FROM `campaign_types`" . $condition);
    ...
    if ($total > 0) {
        ...
        $select = "SELECT `id`,`name`,`sort_by` FROM `campaign_types`" . $condition . $sort . $number_of_record;
                    ...
                    if (hasRight($filters->L, 'edit') || hasRight($filters->L, 'delete')) {
                        if (isset($filters->Actions) && !empty($filters->Actions) && sizeof($filters->Actions) > 0) {
                            ...
                            if (hasRight($filters->L, 'edit'))
                                $data .= '<a href="' . $admin_url . $filters->T . '_campaign_type?id=' . $result['id'] . '" class="btn btn-sm btn-clean btn-icon" title="Edit"><i class="la la-edit"></i></a>';
