...
if (isset($_POST['postData'])) {
    ...
    $status_array = array_values(config('seo_leads.status.value'));
    ...
    if ((empty($id) || $id == 0) && (!hasRight($user_right_title, 'add'))) {
    ...
    } else if (!is_numeric($source_id) || strlen($source_id) > 10 || $source_id <= 0) {
        echo json_encode(['code' => 422, 'errorField' => 'source_id', 'errorDiv' => 'errorMessageSource', 'errorMessage' => 'Please select a valid option.']);
    } else if ($campaign_id != '' && (!is_numeric($campaign_id) || strlen($campaign_id) > 10 || $campaign_id < 0)) {
        ...
        if ($continueProcessing === false) {
            ...
            if (!empty($id) && is_numeric($id) && $id > 0 && hasRight($user_right_title, 'edit')) {
                ...
                $old_lead_messages_sql = mysqli_query($db, "SELECT `added_by`, `created_at` FROM `seo_lead_messages` WHERE `lead_id`='{$id}' ORDER BY `id` ASC LIMIT 1");
                if ($old_lead_messages_sql && mysqli_num_rows($old_lead_messages_sql) > 0) {
                    ...
                    mysqli_query($db, "DELETE FROM `seo_lead_messages` WHERE `lead_id`='{$id}'");
                }

                $update = "UPDATE `seo_leads` SET `date`='{$date}',`sales_person_id`='{$sales_person_id}',`source_id`='{$source_id}',`campaign_id`='{$campaign_id}',`status`='{$status}',`business_name`='{$business_name}',`respondent_name`='{$respondent_name}',`email`='{$email}',`dial_code`='{$dial_code}',`contact_no`='{$contact_no}',`iso`='{$iso}',`other_dial_code`='{$other_dial_code}',`other_contact_no`='{$other_contact_no}',`other_iso`='{$other_iso}',`fax`='{$fax}',`updated_by`='{$user_id}' WHERE `id`='{$id}' AND `company_id`='{$company_id}' AND `branch_id`='{$branch_id}'";
                mysqli_query($db, $update);
                foreach ($object->data as $row => $data) {
                    $insert = "INSERT INTO `seo_lead_messages`(`id`, `lead_id`, `respondent_message`, `our_message`, `our_note`, `company_id`, `branch_id`, `added_by`, `created_at`, `updated_by`, `updated_at`) VALUES (NULL,'{$id}','{$data['respondent_message']}','{$data['our_message']}','{$data['our_note']}','{$company_id}','{$branch_id}','{$added_by}','{$created_at}','{$updated_by}','{$updated_at}')";
                ...
                foreach ($object->data as $row => $data) {
                    $insert = "INSERT INTO `seo_lead_messages`(`id`, `lead_id`, `respondent_message`, `our_message`, `our_note`, `company_id`, `branch_id`, `added_by`) VALUES (NULL,'{$insert_id}','{$data['respondent_message']}','{$data['our_message']}','{$data['our_note']}','{$company_id}','{$branch_id}','{$user_id}')";
                    mysqli_query($db, $insert);
                }
                $code = 200;
                $toasterClass = 'success';
