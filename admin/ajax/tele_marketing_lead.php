...
if (isset($_POST['postData'])) {
    ...
    $status_array = array_values(config('tele_marketing_leads.status.value'));
    ...
    if ((empty($id) || $id == 0) && (!hasRight($user_right_title, 'add'))) {
        ...
        if ($continueProcessing === false) {
            ...
            if (!empty($id) && is_numeric($id) && $id > 0 && hasRight($user_right_title, 'edit')) {
                ...
                $old_lead_messages_sql = mysqli_query($db, "SELECT `added_by`, `created_at` FROM `tele_marketing_lead_messages` WHERE `lead_id`='{$id}' ORDER BY `id` ASC LIMIT 1");
                if ($old_lead_messages_sql && mysqli_num_rows($old_lead_messages_sql) > 0) {
                    ...
                    mysqli_query($db, "DELETE FROM `tele_marketing_lead_messages` WHERE `lead_id`='{$id}'");
                }

                $update = "UPDATE `tele_marketing_leads` SET `date`='{$date}',`sales_person_id`='{$sales_person_id}',`status`='{$status}',`business_name`='{$business_name}',`respondent_name`='{$respondent_name}',`email`='{$email}',`dial_code`='{$dial_code}',`contact_no`='{$contact_no}',`iso`='{$iso}',`other_dial_code`='{$other_dial_code}',`other_contact_no`='{$other_contact_no}',`other_iso`='{$other_iso}',`fax`='{$fax}',`updated_by`='{$user_id}' WHERE `id`='{$id}' AND `company_id`='{$company_id}' AND `branch_id`='{$branch_id}'";
                mysqli_query($db, $update);
                foreach ($object->data as $row => $data) {
                    $insert = "INSERT INTO `tele_marketing_lead_messages`(`id`, `lead_id`, `respondent_message`, `our_message`, `our_note`, `company_id`, `branch_id`, `added_by`, `created_at`, `updated_by`, `updated_at`) VALUES (NULL,'{$id}','{$data['respondent_message']}','{$data['our_message']}','{$data['our_note']}','{$company_id}','{$branch_id}','{$added_by}','{$created_at}','{$updated_by}','{$updated_at}')";
                ...
                $insert = "INSERT INTO `tele_marketing_leads`(`id`, `date`, `sales_person_id`, `status`, `business_name`, `respondent_name`, `email`, `dial_code`, `contact_no`, `iso`, `other_dial_code`, `other_contact_no`, `other_iso`, `fax`, `company_id`, `branch_id`, `added_by`) VALUES (NULL,'{$date}','{$sales_person_id}','{$status}','{$business_name}','{$respondent_name}','{$email}','{$dial_code}','{$contact_no}','{$iso}','{$other_dial_code}','{$other_contact_no}','{$other_iso}','{$fax}','{$company_id}','{$branch_id}','{$user_id}')";
                mysqli_query($db, $insert);
                $insert_id = mysqli_insert_id($db);
                foreach ($object->data as $row => $data) {
                    $insert = "INSERT INTO `tele_marketing_lead_messages`(`id`, `lead_id`, `respondent_message`, `our_message`, `our_note`, `company_id`, `branch_id`, `added_by`) VALUES (NULL,'{$insert_id}','{$data['respondent_message']}','{$data['our_message']}','{$data['our_note']}','{$company_id}','{$branch_id}','{$user_id}')";
                    mysqli_query($db, $insert);
                }
                $code = 200;
                $toasterClass = 'success';
