...
if (isset($_POST['delete_lead'])) {
    if (hasRight('lead', 'delete')) {
        $delete = htmlentities($_POST['delete_lead']);

        if (mysqli_query($db, "UPDATE `leads` SET `deleted_by`='{$global_user_id}', `deleted_at`='{$deleted_at}' WHERE `id`='{$delete}' AND `company_id`='{$global_company_id}' AND `branch_id`='{$global_branch_id}'")) {
            echo json_encode(["code" => 200, "toasterClass" => 'success', "responseMessage" => 'Record has been deleted.']);
        }
        ...
        if ($employee_info->user_type != config('users.type.value.super_admin') && $employee_info->user_type != config('users.type.value.admin')) {
            if (mysqli_query($db, "UPDATE `leads` SET `deleted_by`='{$global_user_id}', `deleted_at`='{$deleted_at}' WHERE `id`='{$delete}' AND `user_id`='{$global_user_id}' AND `company_id`='{$global_company_id}' AND `branch_id`='{$global_branch_id}'") && mysqli_affected_rows($db) > 0) {
                echo json_encode(["code" => 200, "toasterClass" => 'success', "responseMessage" => 'Record has been deleted.']);
            } else {
                echo json_encode(["code" => 200, "toasterClass" => 'warning', "responseMessage" => 'Sorry! You have no right to delete record.']);
            }
        } else {
            if (mysqli_query($db, "UPDATE `leads` SET `deleted_by`='{$global_user_id}', `deleted_at`='{$deleted_at}' WHERE `id`='{$delete}' AND `company_id`='{$global_company_id}' AND `branch_id`='{$global_branch_id}'") && mysqli_affected_rows($db) > 0) {
                echo json_encode(["code" => 200, "toasterClass" => 'success', "responseMessage" => 'Record has been deleted.']);
            } else {
                echo json_encode(["code" => 405, "toasterClass" => 'warning', "responseMessage" => 'Sorry! You have no right to delete record.']);
            }
...
if (isset($_POST['delete_sources_lead'])) {
    if (hasRight('bd_lead', 'delete')) {
        ...
        if ($employee_info->user_type != config('users.type.value.super_admin') && $employee_info->user_type != config('users.type.value.admin')) {
            ...
            if (mysqli_query($db, "UPDATE `sources_lead_details` SET `deleted_by`='{$global_user_id}', `deleted_at`='{$deleted_at}' WHERE `id`='{$delete}' AND `company_id`='{$global_company_id}' AND `branch_id`='{$global_branch_id}'") && mysqli_affected_rows($db) > 0) {
                echo json_encode(["code" => 200, "toasterClass" => 'success', "responseMessage" => 'Record has been deleted.']);
            } else {
                echo json_encode(["code" => 405, "toasterClass" => 'warning', "responseMessage" => 'Sorry! You have no right to delete record.']);
            }
