...
if (isset($_POST['delete_tele_marketing_lead'], $_POST['user_right_title']) && !empty($_POST['delete_tele_marketing_lead']) && !empty($_POST['user_right_title']) && is_numeric($_POST['delete_tele_marketing_lead'])) {
    if (hasRight($_POST['user_right_title'], 'delete')) {
        ...
        $id = htmlentities($_POST['delete_tele_marketing_lead']);
        if (mysqli_query($db, "UPDATE `tele_marketing_leads` SET `deleted_by`='{$global_user_id}', `deleted_at`='{$deleted_at}' WHERE `id`='{$id}' AND `company_id`='{$global_company_id}' AND `branch_id`='{$global_company_id}'".$condition)) {
            echo json_encode(["code" => 200, "toasterClass" => 'success', "responseMessage" => 'Record has been deleted.']);
        } else {
            echo json_encode(["code" => 405, "toasterClass" => 'error', "responseMessage" => 'Unexpected error.']);
        }
