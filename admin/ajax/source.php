...
if (isset($_POST['postData'])) {
    ...
    $type_array = array_values(config('sources.type.value'));
    ...
    if ((empty($id) || $id == 0) && (!hasRight($user_right_title, 'add'))) {
        ...
        if (mysqli_num_rows($checkExist) > 0) {
            ...
            if (!empty($id) && $id > 0) {
                ...
                $query = "INSERT INTO `sources`(`id`, `name`, `sort_by`, `type`, `company_id`, `branch_id`, `added_by`) VALUES (NULL, '{$name}', '{$sort_by}', '{$type}', '{$company_id}', '{$branch_id}', '{$user_id}')";
                $form_reset = true;
            }
