                                ...
                                <div class="card card-custom box-shadow-none">
                                    <div class="card-header flex-wrap py-5">
                                        <div class="card-title">
                                            <h3 class="card-label"> <?php echo $pageHeading; ?> </h3>
                                        </div>
                                        <div class="card-toolbar">
                                            <?php
                                            if (hasRight($user_right_title, 'add')) {
                                                echo '<a href="' . $admin_url . 'tele_marketing_lead" class="btn btn-primary font-weight-bolder">'.config('lang.button.title.new_record').'</a>';
                                            }
                                            ?>
                                        </div>
                                    </div>
...
<script>
    ...
    function getAllPageData(filter) {
        loader(true);
        $.ajax({
            type: "POST", url: 'ajax/fetch/tele_marketing_leads.php',
            data: {'filters': filter},
            ...
            if (result.value) {
                ...
                if (D) {
                    ...
                    $.ajax({
                        ...
                        data: "delete_tele_marketing_lead=" + id + "&user_right_title=<?php echo $user_right_title; ?>",
                        success: function (resPonse) {
                            if (resPonse !== undefined && resPonse != '') {
                                var obj = JSON.parse(resPonse);
