                                                                    ...
                                                                    <div class="col-md-4">
                                                                        <div class="form-group">
                                                                            ...
                                                                            <select tabindex="35"
                                                                                ...
                                                                                <?php
                                                                                $select = "SELECT * FROM `seo_campaigns` WHERE `company_id`='{$global_company_id}' AND `branch_id`='{$global_branch_id}' AND `deleted_at` IS NULL ORDER BY `from` DESC";
                                                                                $query = mysqli_query($db, $select);
                                                                                if (mysqli_num_rows($query) > 0) {
                                                                                    while ($result = mysqli_fetch_object($query)) {
                                                                                        $selected = '';
                                                                                        ...
                                                                                        ?>
                                                                                        <option <?php echo $selected; ?>
                                                                                                value="<?php echo $result->id; ?>"><?php echo $result->name.' ('. date('d-M-Y', strtotime($result->from)).' To '. date('d-M-Y', strtotime($result->to)).') '; ?> </option>
                                                                                        <?php
                                                                                    }
                                                                                }
                                                                                ?>
                                                                    ...
                                                                    <div class="col-md-4">
                                                                        <div class="form-group">
                                                                            ...
                                                                            <select tabindex="40"
                                                                                ...
                                                                                <?php
                                                                                foreach (config('seo_leads.status.title') as $key => $val) {
                                                    ...
                                                    <div class="mb-2">
                                                        <div id="Data_Holder_Parent_Div">
                                                            ...
                                                            <div id="Data_Holder_Child_Div" class="mt-7 mb-7"
                                                                ...
                                                                $i = 1;
                                                                if (!empty($id)) {
                                                                    $query_lead_messages = mysqli_query($db, "SELECT * FROM `seo_lead_messages` WHERE `lead_id`='{$id}' ORDER BY `id` ASC");
...
?>
    <script>
        ...
        function saveFORM() {
            ...
            var statusArray = [<?php echo '"' . implode('","', array_values(config('seo_leads.status.value'))) . '"' ?>];
            ...
            date.style.borderColor = select2_sales_person_id_container.style.borderColor = select2_source_id_container.style.borderColor = select2_campaign_id_container.style.borderColor = select2_status_container.style.borderColor = '#E4E6EF';
            ...
            errorMessageDate.innerText = errorMessageSalesPersonId.innerText = errorMessageSource.innerText = errorMessageCampaignId.innerText = errorMessageStatus.innerText = '';
            ...
            if (id.value == 0 && A == '') {
                ...
                toasterTrigger(toasterType, error);
                return false;
            } else if (source_id.value == '' || source_id.value <= 0) {
                select2_source_id_container.style.borderColor = '#F00';
                error = "Source field is required.";
                errorMessageSource.innerText = error;
                toasterTrigger(toasterType, error);
                return false;
            } else if (isNaN(source_id.value) === true || source_id.value.length > 10) {
                select2_source_id_container.style.borderColor = '#F00';
                error = "Please select a valid option.";
                errorMessageSource.innerText = error;
                toasterTrigger(toasterType, error);
                return false;
            } else if (campaign_id.value != '' && (isNaN(campaign_id.value) === true || campaign_id.value.length > 10 || campaign_id.value < 0)) {
                ...
                if (continueProcessing === false) {
                    ...
                    loader(true);
                    $.ajax({
                        type: "POST", url: "ajax/seo_lead.php",
