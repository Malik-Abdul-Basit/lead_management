                                                        ...
                                                        <div class="col-md-3 my-2 my-md-0">
                                                            <div class="form-group">
                                                                ...
                                                                <select id="BG_SalesPersonFilter"
                                                                    ...
                                                                    <?php
                                                                    $select = "SELECT sp.id, CONCAT(sp.first_name,' ',sp.last_name,' (',sp.email,')') AS name FROM sales_persons AS sp INNER JOIN seo_leads AS l ON sp.id=l.sales_person_id WHERE sp.company_id='{$global_company_id}' AND sp.branch_id='{$global_branch_id}' AND sp.deleted_at IS NULL GROUP BY sp.id ORDER BY sp.first_name ASC";
                                                        ...
                                                        <div class="col-md-2 my-2 my-md-0">
                                                            <div class="form-group">
                                                                ...
                                                                <select id="BG_SourceFilter"
                                                                    ...
                                                                    <?php
                                                                    $select = "SELECT s.id, s.name FROM sources AS s INNER JOIN seo_leads AS l ON s.id=l.source_id WHERE s.type='{$type}' AND  s.company_id='{$global_company_id}' AND s.branch_id='{$global_branch_id}' AND s.deleted_at IS NULL AND l.company_id='{$global_company_id}' AND l.branch_id='{$global_branch_id}' AND l.deleted_at IS NULL GROUP BY s.id ORDER BY s.sort_by ASC";
                                                                    $query = mysqli_query($db, $select);
                                                                    if (mysqli_num_rows($query) > 0) {
                                                                        while ($result = mysqli_fetch_object($query)) {
                                                                            ?>
...
<script>
    ...
    function getData() {
        ...
        var filter = {
            ...
            'Filter': [
                ...
                {'field': 'l.source_id', 'value': BG_SourceFilter.value},
    ...
    function getAllPageData(filter) {
        loader(true);
        $.ajax({
            type: "POST", url: 'ajax/fetch/seo_leads.php',
            data: {'filters': filter},
            ...
            if (result.value) {
                ...
                if (D) {
                    ...
                    $.ajax({
                        ...
                        data: "delete_seo_lead=" + id + "&user_right_title=<?php echo $user_right_title; ?>",
                        success: function (resPonse) {
                            if (resPonse !== undefined && resPonse != '') {
                                var obj = JSON.parse(resPonse);
